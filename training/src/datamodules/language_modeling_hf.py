...
class LMDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.dataset_train, self.dataset_val, self.dataset_test = [
            LMDataset(concat_ids[split], seq_len=self.max_length) for split in ["train", "validation", "test"]
        ]
        ...
        if self.shuffle and self.fault_tolerant:
            ...
            if self.ddp and self.fast_forward_epochs is not None and self.fast_forward_batches is not None:
                sampler.load_state_dict(
                    {"epoch": self.fast_forward_epochs, "counter": self.fast_forward_batches * self.batch_size}
                )
        else:
    ...
    def _data_loader(self, dataset: Dataset, batch_size: int, shuffle: bool = False, sampler=None) -> DataLoader:
        ...
        # sampler = SubsetRandomSampler(indices=torch.randperm(len(dataset))[: 10 * batch_size])
