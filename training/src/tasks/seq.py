...
class SequenceModel(LightningModule):
    ...
    def warmstart(self):
        if self.cfg.train.get("warmstart", None) is not None:
            ...
            if self.cfg.train.warmstart.get("post_process", None) is not None:
                state_dict = hydra.utils.instantiate(self.cfg.train.warmstart.post_process, state_dict)
    ...
    def step(self, batch: Any, is_train=True):
        ...
        # shape: (batch_size, seq_len, vocab_size) -> (batch_size * seq_len, vocab_size)
        output = output.flatten(end_dim=1)
        # shape: (batch_size, seq_len) -> (batch_size * seq_len)
        y = y.flatten()
        # torch.nn.CrossEntropyLoss expects the input to be (batch, num_classes) and the target to be (batch,)
    ...
    def shared_step(self, batch: Any, batch_idx: int, phase="train"):
        ...
        self.log(f"{phase}/loss", loss, on_step=log_on_step, on_epoch=True, prog_bar=False, sync_dist=True)
    ...
    def configure_optimizers(self):
        if "optimizer_param_grouping" in self.cfg.train:  # Set zero weight decay for some params
            ...
            parameters = (
                self.parameters()
            )  # [21-09-08] AG: this will train task specific parameters such as Retrieval head for AAN
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        # checkpoint["loops"]["fit_loop"]["epoch_loop.state_dict"]["_batches_that_stepped"] = checkpoint["loops"][
        #     "fit_loop"
        # ]["epoch_loop.batch_loop.optimizer_loop.optim_progress"]["optimizer"]["step"]["total"]["completed"]
